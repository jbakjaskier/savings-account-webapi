...
{
    public Task<OperationResponse<AccountResponse, ActionFailure>> CreateSavingsAccount(ValidatedSavingsAccountRequest account, CancellationToken cancellationToken);
...
public class AccountRepository (AccountDbContext context, IOptions<SavingsAccountCreationConfiguration> savingsAccountCreationOptions, ILogger<AccountRepository> logger): IAccountRepository
{
    public async Task<OperationResponse<AccountResponse, ActionFailure>> CreateSavingsAccount(ValidatedSavingsAccountRequest account, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (existingAccountCount is not null && existingAccountCount >= savingsAccountCreationOptions.Value.MaxAccountsPerCustomer)
            {
                logger.LogInformation("The {customerNumber} has exceeded the maximum number of accounts allowed to be created per customer as they have {existingAccountCount} accounts and only {maxAccountPerCustomer} are allowed", account.CustomerNumber, existingAccountCount, savingsAccountCreationOptions.Value.MaxAccountsPerCustomer);
                
                return new OperationResponse<AccountResponse, ActionFailure>.FailedOperation(
                    new ActionFailure(ErrorCode.ExceededMaxNumberOfAccounts, $"The Customer has exceeded the maximum number of accounts allowed to be created per customer"));
            }
        ...
        catch (Exception exception)
        {
            ...
            return new OperationResponse<AccountResponse, ActionFailure>.FailedOperation(
                new ActionFailure(ErrorCode.UnknownFailure, "An error occurred while creating the savings account. Please contact the support team if the issue persists")
            );
    ...
    public async Task<OperationResponse<List<AccountResponse>, ActionFailure>> GetAccountsForCustomer(long customerNumber)
    {
        try
        {
            var customerInDb = await context
                ...
                .Include(x => x.Accounts)
                .AsNoTracking()
                .SingleOrDefaultAsync(x => x.CustomerNumber == customerNumber);
            ...
            {
                return new OperationResponse<List<AccountResponse>, ActionFailure>.FailedOperation(
                    new ActionFailure(ErrorCode.CustomerDoesNotHaveAnyAccounts, $"The Customer Number does not have any accounts")
                );
            }
            ...
            logger.LogInformation("{numberOfAccounts} were retrieved for customer with {customerNumber}", response.Count, customerNumber);
        ...
        catch (Exception exception)
        {
            ...
            return new OperationResponse<List<AccountResponse>, ActionFailure>.FailedOperation(
                new ActionFailure(ErrorCode.UnknownFailure, "An error occurred while getting the savings account for a customer. Please contact the support team if the issue persists")
            );
        }
    }
